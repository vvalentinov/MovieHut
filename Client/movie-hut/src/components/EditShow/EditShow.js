...
import { Option } from '../EditMovie/Option/Option';
import { ActorContext } from '../../contexts/ActorContext';
import { CelebrityOption } from '../EditMovie/CelebrityOption/CelebrityOption';
import { AddedCelebrity } from '../EditMovie/AddedCelebrity/AddedCelebrity';
...
export const EditShow = () => {
        ...
        showService.getOne(showId)
            .then(res => {
                setInputData({
                    title: res.title,
                    plot: res.plot,
                    released: res.released.split('T')[0],
                    posterUrl: res.posterUrl,
                    trailerUrl: res.trailerUrl,
                    seasonsCount: res.seasonsCount,
                })
                setImageData((state) => ({ ...state, imageUrl: res.posterUrl }));
                setVisualizationImageUrl(res.posterUrl);
                //Set the added actors
                setAddedActors(res.actors.map(x => x.id));
                //Set the added directors
                //setAddedDirectors(res.directors.map(x => x.id));
                //Set the checked genres
            }).catch(err => {
                alert(err);
            })
    }, [])
    const onSubmit = (e) => {
        ...
        formData.append('url', imageData.imageUrl);
        //Start spinner
        setIsLoading(true);
        ...
        if (imageData.imageFile) {
            imageService
                ...
                .then(imgRes => {
                    showService
                        .edit({ ...inputData, id: showId, posterUrl: imgRes.imageUrl })
                        .then((res) => {
                            //Update context
                            //update(res);
                            //Stop spinner
                            setIsLoading(false);
                            navigate(`/shows/details/${showId}`);
                        })
                        .catch((err) => {
                            ...
                            <form encType='multipart/form-data'
                                ...
                                <div className='form-outline'>
                                    <input
                                        ...
                                        name='seasonsCount'
                                        className='form-control form-control-lg'
                                        placeholder='Enter the count of the seasons'
                                ...
                                {/*Seasons count alert*/}
                                {errors.seasonsCount &&
                                    <Alert message="The count of the seasons must be between 1 and 500 minutes." />
                                }
                                {/*Poster input*/}
                                <div>
                                    <input
                                    ...
                                    <div className='col-4'>
                                        <div className='text-center text-lg-start mt-2 pt-2'>
                                            <button
                                                ...
                                                disabled={!isValidForm || (!inputData.title || !inputData.plot || !inputData.released || !inputData.trailerUrl || !inputData.seasonsCount || !visualizationImageUrl || addedActors.length <= 0 || addedDirectors.length <= 0 || !areChecked.some(x => x === true))}
                                                style={{
                                                    paddingLeft: '2.5rem',
                                                    paddingRight: '2.5rem',
